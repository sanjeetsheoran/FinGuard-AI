"""
FinGuard AI - Identity Feature Experiment 2

Experiment 2:
    Experiment 1 features
    +
    Identity dataset features
    +
    Identity availability features

Goal:
    Determine whether identity information improves the
    current best model.

Rules:
    - Same predefined train/validation split.
    - Test set is NOT used.
    - Preprocessor is fitted on TRAIN only.
    - Validation is transformed only.
    - isFraud is never used as an input feature.
    - TransactionID is used only as a join key.
"""

from pathlib import Path
import json
import sys

import joblib
import pandas as pd

from sklearn.metrics import (
    average_precision_score,
    classification_report,
    confusion_matrix,
    f1_score,
    precision_score,
    recall_score,
    roc_auc_score,
)

from xgboost import XGBClassifier


# ============================================================
# Project Setup
# ============================================================

PROJECT_ROOT = Path(__file__).resolve().parents[1]

if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))


# ============================================================
# Paths
# ============================================================

TRANSACTION_FILE = (
    PROJECT_ROOT
    / "ml"
    / "datasets"
    / "raw"
    / "train_transaction.csv"
)

IDENTITY_FILE = (
    PROJECT_ROOT
    / "ml"
    / "datasets"
    / "raw"
    / "train_identity.csv"
)

SPLIT_FILE = (
    PROJECT_ROOT
    / "ml"
    / "datasets"
    / "splits"
    / "transaction_split.csv"
)

BASELINE_METRICS_FILE = (
    PROJECT_ROOT
    / "ml"
    / "metrics"
    / "experiment_1_metrics.json"
)

METRICS_DIR = (
    PROJECT_ROOT
    / "ml"
    / "metrics"
)

MODEL_DIR = (
    PROJECT_ROOT
    / "ml"
    / "models"
    / "experiment_2"
)

METRICS_DIR.mkdir(
    parents=True,
    exist_ok=True,
)

MODEL_DIR.mkdir(
    parents=True,
    exist_ok=True,
)


# ============================================================
# Configuration
# ============================================================

ID_COLUMN = "TransactionID"
TARGET = "isFraud"

RANDOM_STATE = 42

N_ESTIMATORS = 300
MAX_DEPTH = 6
LEARNING_RATE = 0.08
SUBSAMPLE = 0.8
COLSAMPLE_BYTREE = 0.8

THRESHOLD = 0.50


# ============================================================
# Transaction Features
# ============================================================

TRANSACTION_FEATURES = [
    "TransactionDT",
    "TransactionAmt",
    "ProductCD",

    "card1",
    "card2",
    "card3",
    "card4",
    "card5",
    "card6",

    "addr1",
    "addr2",
    "dist1",
    "dist2",

    "P_emaildomain",
    "R_emaildomain",

    "M1",
    "M2",
    "M3",
    "M4",
    "M5",
    "M6",
    "M7",
    "M8",
    "M9",
]


# ============================================================
# Experiment 1 Features
# ============================================================

ENGINEERED_FEATURES = [
    "TransactionAmtLog",
    "TransactionAmtBucket",
    "TransactionHour",
    "TransactionDay",
]


# ============================================================
# Identity Features
# ============================================================

IDENTITY_NUMERICAL_FEATURES = [
    "id_01",
    "id_02",
    "id_03",
    "id_04",
    "id_05",
    "id_06",
    "id_07",
    "id_08",
    "id_09",
    "id_10",
    "id_11",
    "id_13",
    "id_14",
    "id_15",
    "id_17",
    "id_19",
    "id_20",
    "id_21",
    "id_22",
    "id_24",
    "id_25",
    "id_26",
    "id_32",
]


IDENTITY_CATEGORICAL_FEATURES = [
    "id_12",
    "id_16",
    "id_18",
    "id_23",
    "id_27",
    "id_28",
    "id_29",
    "id_30",
    "id_31",
    "id_33",
    "id_34",
    "id_35",
    "id_36",
    "id_37",
    "id_38",
    "DeviceType",
    "DeviceInfo",
]


IDENTITY_AVAILABILITY_FEATURES = [
    "HasIdentity",
    "IdentityMissingCount",
    "IdentityMissingRatio",
]


# ============================================================
# Utility
# ============================================================

def section(title: str) -> None:

    print("\n" + "=" * 70)
    print(title)
    print("=" * 70)


# ============================================================
# Feature Engineering
# ============================================================

def engineer_transaction_features(
    df: pd.DataFrame,
) -> pd.DataFrame:

    result = df.copy()

    # --------------------------------------------------------
    # Log transaction amount
    # --------------------------------------------------------

    result["TransactionAmtLog"] = (
        result["TransactionAmt"]
        .clip(lower=0)
        .apply(lambda x: __import__("math").log1p(x))
    )

    # --------------------------------------------------------
    # Transaction amount bucket
    # --------------------------------------------------------

    def amount_bucket(value):

        if pd.isna(value):
            return "unknown"

        if value <= 25:
            return "very_low"

        if value <= 50:
            return "low"

        if value <= 100:
            return "medium"

        if value <= 250:
            return "high"

        if value <= 500:
            return "very_high"

        if value <= 1000:
            return "premium"

        return "extreme"

    result["TransactionAmtBucket"] = (
        result["TransactionAmt"]
        .apply(amount_bucket)
    )

    # --------------------------------------------------------
    # Transaction hour
    # --------------------------------------------------------

    result["TransactionHour"] = (
        (result["TransactionDT"] // 3600)
        % 24
    )

    # --------------------------------------------------------
    # Transaction day
    # --------------------------------------------------------

    result["TransactionDay"] = (
        result["TransactionDT"] // 86400
    )

    return result


# ============================================================
# Identity Features
# ============================================================

def add_identity_features(
    transaction_df: pd.DataFrame,
    identity_df: pd.DataFrame,
) -> pd.DataFrame:

    section(
        "MERGING IDENTITY FEATURES"
    )

    if identity_df[
        ID_COLUMN
    ].duplicated().any():

        raise ValueError(
            "Duplicate TransactionIDs found "
            "in identity dataset."
        )

    result = transaction_df.merge(
        identity_df,
        on=ID_COLUMN,
        how="left",
        validate="one_to_one",
    )

    if len(result) != len(
        transaction_df
    ):

        raise ValueError(
            "Identity merge changed row count."
        )

    # --------------------------------------------------------
    # Identity availability
    #
    # IMPORTANT:
    # Only actual identity columns are used.
    # TARGET is never used here.
    # --------------------------------------------------------

    identity_columns = [
        column
        for column in (
            IDENTITY_NUMERICAL_FEATURES
            + IDENTITY_CATEGORICAL_FEATURES
        )
        if column in result.columns
    ]

    result["HasIdentity"] = (
        result[
            identity_columns
        ]
        .notna()
        .any(axis=1)
        .astype("int8")
    )

    result["IdentityMissingCount"] = (
        result[
            identity_columns
        ]
        .isna()
        .sum(axis=1)
    )

    result["IdentityMissingRatio"] = (
        result[
            identity_columns
        ]
        .isna()
        .sum(axis=1)
        / len(identity_columns)
    )

    print(
        f"Transaction rows : "
        f"{len(transaction_df):,}"
    )

    print(
        f"Merged rows      : "
        f"{len(result):,}"
    )

    print(
        "Identity merge: PASSED"
    )

    return result


# ============================================================
# Load Data
# ============================================================

def load_data() -> pd.DataFrame:

    section(
        "LOADING DATA"
    )

    if not TRANSACTION_FILE.exists():
        raise FileNotFoundError(
            f"Transaction dataset not found:\n"
            f"{TRANSACTION_FILE}"
        )

    if not IDENTITY_FILE.exists():
        raise FileNotFoundError(
            f"Identity dataset not found:\n"
            f"{IDENTITY_FILE}"
        )

    if not SPLIT_FILE.exists():
        raise FileNotFoundError(
            f"Split manifest not found:\n"
            f"{SPLIT_FILE}"
        )

    # --------------------------------------------------------
    # Load split
    # --------------------------------------------------------

    split_df = pd.read_csv(
        SPLIT_FILE,
        usecols=[
            ID_COLUMN,
            "split",
        ],
    )

    print(
        f"Split records: "
        f"{len(split_df):,}"
    )

    # --------------------------------------------------------
    # Load transaction data
    # --------------------------------------------------------

    transaction_columns = [
        ID_COLUMN,
        TARGET,
        *TRANSACTION_FEATURES,
    ]

    transaction_columns = list(
        dict.fromkeys(
            transaction_columns
        )
    )

    print(
        "Loading transaction features..."
    )

    transaction_df = pd.read_csv(
        TRANSACTION_FILE,
        usecols=transaction_columns,
    )

    print(
        f"Transaction rows: "
        f"{len(transaction_df):,}"
    )

    # --------------------------------------------------------
    # Load identity data
    # --------------------------------------------------------

    identity_columns = [
        ID_COLUMN,
        *IDENTITY_NUMERICAL_FEATURES,
        *IDENTITY_CATEGORICAL_FEATURES,
    ]

    identity_columns = list(
        dict.fromkeys(
            identity_columns
        )
    )

    print(
        "Loading identity features..."
    )

    identity_df = pd.read_csv(
        IDENTITY_FILE,
        usecols=identity_columns,
    )

    print(
        f"Identity rows: "
        f"{len(identity_df):,}"
    )

    # --------------------------------------------------------
    # Validate identity key
    # --------------------------------------------------------

    identity_duplicates = (
        identity_df[
            ID_COLUMN
        ]
        .duplicated()
        .sum()
    )

    if identity_duplicates != 0:

        raise ValueError(
            "Duplicate TransactionIDs in identity dataset."
        )

    # --------------------------------------------------------
    # Add split assignment
    # --------------------------------------------------------

    transaction_df = transaction_df.merge(
        split_df,
        on=ID_COLUMN,
        how="inner",
        validate="one_to_one",
    )

    if len(transaction_df) != len(
        split_df
    ):

        raise ValueError(
            "Split manifest coverage mismatch."
        )

    # --------------------------------------------------------
    # Add transaction engineered features
    # --------------------------------------------------------

    section(
        "APPLYING TRANSACTION FEATURE ENGINEERING"
    )

    transaction_df = engineer_transaction_features(
        transaction_df
    )

    print(
        "Engineered features added:"
    )

    for feature in ENGINEERED_FEATURES:

        print(
            f"- {feature}"
        )

    # --------------------------------------------------------
    # Add identity features
    # --------------------------------------------------------

    transaction_df = add_identity_features(
        transaction_df,
        identity_df,
    )

    return transaction_df


# ============================================================
# Prepare Splits
# ============================================================

def prepare_splits(
    df: pd.DataFrame,
):

    section(
        "PREPARING TRAIN / VALIDATION SPLITS"
    )

    train_df = df[
        df["split"] == "train"
    ].copy()

    validation_df = df[
        df["split"] == "validation"
    ].copy()

    print(
        f"Train rows      : "
        f"{len(train_df):,}"
    )

    print(
        f"Validation rows : "
        f"{len(validation_df):,}"
    )

    # --------------------------------------------------------
    # Input features
    # --------------------------------------------------------

    feature_columns = [
        *TRANSACTION_FEATURES,
        *ENGINEERED_FEATURES,
        *IDENTITY_NUMERICAL_FEATURES,
        *IDENTITY_CATEGORICAL_FEATURES,
        *IDENTITY_AVAILABILITY_FEATURES,
    ]

    feature_columns = [
        column
        for column in feature_columns
        if column in df.columns
    ]

    # --------------------------------------------------------
    # Explicit leakage checks
    # --------------------------------------------------------

    forbidden_columns = {
        TARGET,
        ID_COLUMN,
        "split",
    }

    leakage_columns = (
        set(feature_columns)
        & forbidden_columns
    )

    if leakage_columns:

        raise ValueError(
            "Leakage detected in feature columns: "
            f"{leakage_columns}"
        )

    X_train = train_df[
        feature_columns
    ].copy()

    X_validation = validation_df[
        feature_columns
    ].copy()

    y_train = train_df[
        TARGET
    ].copy()

    y_validation = validation_df[
        TARGET
    ].copy()

    print(
        f"\nInput feature columns: "
        f"{len(feature_columns)}"
    )

    print(
        "Target exclusion: PASSED"
    )

    return (
        X_train,
        X_validation,
        y_train,
        y_validation,
    )


# ============================================================
# Class Imbalance
# ============================================================

def calculate_scale_pos_weight(
    y: pd.Series,
) -> float:

    negative = int(
        (y == 0).sum()
    )

    positive = int(
        (y == 1).sum()
    )

    if positive == 0:

        raise ValueError(
            "No fraud examples in training data."
        )

    return negative / positive


# ============================================================
# Train Model
# ============================================================

def train_model(
    X_train_processed,
    y_train,
    scale_pos_weight,
):

    section(
        "TRAINING EXPERIMENT 2"
    )

    model = XGBClassifier(
        n_estimators=N_ESTIMATORS,
        max_depth=MAX_DEPTH,
        learning_rate=LEARNING_RATE,
        subsample=SUBSAMPLE,
        colsample_bytree=COLSAMPLE_BYTREE,
        objective="binary:logistic",
        eval_metric="aucpr",
        tree_method="hist",
        random_state=RANDOM_STATE,
        scale_pos_weight=scale_pos_weight,
        n_jobs=-1,
    )

    print(
        f"scale_pos_weight: "
        f"{scale_pos_weight:.4f}"
    )

    print(
        f"n_estimators: "
        f"{N_ESTIMATORS}"
    )

    print(
        f"max_depth: "
        f"{MAX_DEPTH}"
    )

    print(
        f"learning_rate: "
        f"{LEARNING_RATE}"
    )

    model.fit(
        X_train_processed,
        y_train,
    )

    print(
        "\nExperiment 2 model training completed."
    )

    return model


# ============================================================
# Evaluation
# ============================================================

def evaluate_model(
    model,
    X_validation_processed,
    y_validation,
):

    section(
        "EXPERIMENT 2 VALIDATION"
    )

    probabilities = model.predict_proba(
        X_validation_processed
    )[:, 1]

    predictions = (
        probabilities >= THRESHOLD
    ).astype(int)

    pr_auc = average_precision_score(
        y_validation,
        probabilities,
    )

    roc_auc = roc_auc_score(
        y_validation,
        probabilities,
    )

    precision = precision_score(
        y_validation,
        predictions,
        zero_division=0,
    )

    recall = recall_score(
        y_validation,
        predictions,
        zero_division=0,
    )

    f1 = f1_score(
        y_validation,
        predictions,
        zero_division=0,
    )

    cm = confusion_matrix(
        y_validation,
        predictions,
    )

    print(
        f"\nPR-AUC    : "
        f"{pr_auc:.6f}"
    )

    print(
        f"ROC-AUC   : "
        f"{roc_auc:.6f}"
    )

    print(
        f"Precision : "
        f"{precision:.6f}"
    )

    print(
        f"Recall    : "
        f"{recall:.6f}"
    )

    print(
        f"F1-Score  : "
        f"{f1:.6f}"
    )

    print(
        "\nConfusion Matrix:"
    )

    print(cm)

    print(
        "\nClassification Report:"
    )

    print(
        classification_report(
            y_validation,
            predictions,
            target_names=[
                "Legitimate",
                "Fraud",
            ],
            zero_division=0,
        )
    )

    metrics = {
        "experiment": "experiment_2",
        "description": (
            "Experiment 1 transaction/time features "
            "+ identity features"
        ),
        "threshold": THRESHOLD,
        "random_state": RANDOM_STATE,
        "n_estimators": N_ESTIMATORS,
        "max_depth": MAX_DEPTH,
        "learning_rate": LEARNING_RATE,
        "subsample": SUBSAMPLE,
        "colsample_bytree": COLSAMPLE_BYTREE,
        "pr_auc": float(pr_auc),
        "roc_auc": float(roc_auc),
        "precision": float(precision),
        "recall": float(recall),
        "f1_score": float(f1),
        "confusion_matrix": cm.tolist(),
        "train_rows": 413378,
        "validation_rows": 88581,
    }

    return metrics


# ============================================================
# Compare With Experiment 1
# ============================================================

def compare_with_experiment_1(
    experiment_2_metrics: dict,
):

    section(
        "EXPERIMENT 1 VS EXPERIMENT 2"
    )

    if not BASELINE_METRICS_FILE.exists():

        print(
            "Experiment 1 metrics file not found."
        )

        return

    with open(
        BASELINE_METRICS_FILE,
        "r",
        encoding="utf-8",
    ) as file:

        experiment_1 = json.load(
            file
        )

    metrics = [
        "pr_auc",
        "roc_auc",
        "precision",
        "recall",
        "f1_score",
    ]

    print(
        f"{'Metric':<15}"
        f"{'Experiment 1':>18}"
        f"{'Experiment 2':>18}"
        f"{'Change':>15}"
    )

    print(
        "-" * 66
    )

    for metric in metrics:

        exp1_value = float(
            experiment_1[metric]
        )

        exp2_value = float(
            experiment_2_metrics[metric]
        )

        change = (
            exp2_value
            - exp1_value
        )

        print(
            f"{metric:<15}"
            f"{exp1_value:>18.6f}"
            f"{exp2_value:>18.6f}"
            f"{change:>+15.6f}"
        )

    print()

    exp1_pr_auc = float(
        experiment_1["pr_auc"]
    )

    exp2_pr_auc = float(
        experiment_2_metrics["pr_auc"]
    )

    if exp2_pr_auc > exp1_pr_auc:

        print(
            "Experiment 2 improved PR-AUC."
        )

        print(
            "Decision: KEEP"
        )

    elif exp2_pr_auc < exp1_pr_auc:

        print(
            "Experiment 2 reduced PR-AUC."
        )

        print(
            "Decision: REJECT"
        )

    else:

        print(
            "Experiment 2 produced the same PR-AUC."
        )

        print(
            "Decision: REVIEW"
        )


# ============================================================
# Save Artifacts
# ============================================================

def save_artifacts(
    model,
    preprocessor,
    metrics,
):

    section(
        "SAVING EXPERIMENT 2 ARTIFACTS"
    )

    model_file = (
        MODEL_DIR
        / "xgboost_experiment_2.json"
    )

    preprocessor_file = (
        MODEL_DIR
        / "preprocessor.joblib"
    )

    metrics_file = (
        METRICS_DIR
        / "experiment_2_metrics.json"
    )

    model.save_model(
        model_file
    )

    joblib.dump(
        preprocessor,
        preprocessor_file,
    )

    with open(
        metrics_file,
        "w",
        encoding="utf-8",
    ) as file:

        json.dump(
            metrics,
            file,
            indent=4,
        )

    print(
        f"Model saved:\n"
        f"{model_file}"
    )

    print(
        f"Preprocessor saved:\n"
        f"{preprocessor_file}"
    )

    print(
        f"Metrics saved:\n"
        f"{metrics_file}"
    )


# ============================================================
# Main
# ============================================================

def main():

    section(
        "FinGuard AI - Identity Feature Experiment 2"
    )

    # --------------------------------------------------------
    # Load and engineer data
    # --------------------------------------------------------

    df = load_data()

    # --------------------------------------------------------
    # Prepare train / validation
    # --------------------------------------------------------

    (
        X_train,
        X_validation,
        y_train,
        y_validation,
    ) = prepare_splits(
        df
    )

    # --------------------------------------------------------
    # Build preprocessing
    # --------------------------------------------------------

    section(
        "BUILDING PREPROCESSOR"
    )

    # Import here to keep preprocessing logic centralized.
    from ml.preprocessing.preprocessor import (
        build_preprocessor,
    )

    preprocessor = build_preprocessor(
        X_train
    )

    # --------------------------------------------------------
    # TRAIN ONLY
    # --------------------------------------------------------

    print(
        "Fitting preprocessing on TRAIN only..."
    )

    X_train_processed = (
        preprocessor.fit_transform(
            X_train
        )
    )

    print(
        f"Processed train shape: "
        f"{X_train_processed.shape}"
    )

    # --------------------------------------------------------
    # VALIDATION ONLY
    # --------------------------------------------------------

    print(
        "Transforming validation data..."
    )

    X_validation_processed = (
        preprocessor.transform(
            X_validation
        )
    )

    print(
        f"Processed validation shape: "
        f"{X_validation_processed.shape}"
    )

    # --------------------------------------------------------
    # Class imbalance
    # --------------------------------------------------------

    scale_pos_weight = (
        calculate_scale_pos_weight(
            y_train
        )
    )

    # --------------------------------------------------------
    # Train
    # --------------------------------------------------------

    model = train_model(
        X_train_processed,
        y_train,
        scale_pos_weight,
    )

    # --------------------------------------------------------
    # Evaluate
    # --------------------------------------------------------

    metrics = evaluate_model(
        model,
        X_validation_processed,
        y_validation,
    )

    # --------------------------------------------------------
    # Compare
    # --------------------------------------------------------

    compare_with_experiment_1(
        metrics
    )

    # --------------------------------------------------------
    # Save
    # --------------------------------------------------------

    save_artifacts(
        model,
        preprocessor,
        metrics,
    )

    # --------------------------------------------------------
    # Final
    # --------------------------------------------------------

    section(
        "EXPERIMENT 2 COMPLETE"
    )

    print(
        "Identity features were included."
    )

    print(
        "Preprocessor fitted on TRAIN only."
    )

    print(
        "Validation used only for evaluation."
    )

    print(
        "Test set was NOT used."
    )


# ============================================================
# Entry Point
# ============================================================

if __name__ == "__main__":
    main()